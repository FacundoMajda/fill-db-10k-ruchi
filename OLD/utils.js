...
export function generarMateriasYNotas(nivel) {
  try {
    const materiasPorNivel = obtenerMateriasPorNivel(nivel);
    if (!materiasPorNivel) {
      throw new Error(
        `No se encontraron materias para el nivel educativo '${nivel}'.`
      );
    }
